...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean = b match {
    case Block(b1, b2) => b1.equals(goal) && b2.equals(goal)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case _ => {
      val newNeighbors = for {
        (b, moves) <- initial
        n <- newNeighborsOnly(neighborsWithHistory(b, moves), explored)
      } yield n
      initial ++ from(newNeighbors, explored ++ newNeighbors.map(n => n._1))
    }
  }
